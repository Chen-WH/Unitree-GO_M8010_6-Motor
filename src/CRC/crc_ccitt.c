...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs != 1) {
        mexErrMsgIdAndTxt("crc_ccitt:nrhs", "One input argument required.");
    }
    if (mxIsEmpty(prhs[0])) {
        mexErrMsgIdAndTxt("crc_ccitt:emptyInput", "Input array is empty.");
    }
    if (!mxIsUint8(prhs[0])) {
        mexErrMsgIdAndTxt("crc_ccitt:invalidInputType", "Input array must be uint8.");
    }


	uint8_t *data = (uint8_t *)mxGetData(prhs[0]);
    size_t len = mxGetNumberOfElements(prhs[0]);
	uint16_t crc = crc_ccitt(0, data, len);
    plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT16_CLASS, mxREAL);
	uint16_t *crc_ptr = (uint16_t *)mxGetData(plhs[0]);
	nlhs = 1;
    *crc_ptr = crc;
}
